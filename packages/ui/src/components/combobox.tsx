...
function ComboboxInput({
  ...
  // single mode
  return (
    <div className="relative w-full [--scale-offset:var(--ui-scale-offset-mobile,0rem)] has-disabled:opacity-64 sm:[--scale-offset:var(--ui-scale-offset,0rem)]">
      <ComboboxPrimitive.Input
        className={cn(
          sizeValue === "sm"
      ...
      {showTrigger && (
        <ComboboxTrigger
          className={cn(
            "-translate-y-1/2 absolute top-1/2 inline-flex size-[calc(--spacing(7)+var(--scale-offset))] shrink-0 cursor-pointer items-center justify-center rounded-md border border-transparent opacity-80 outline-none transition-opacity pointer-coarse:after:absolute pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 hover:opacity-100 has-[+[data-slot=combobox-clear]]:hidden [&_svg:not([class*='size-'])]:size-[calc(--spacing(4)+var(--scale-offset)/2)] [&_svg]:pointer-events-none [&_svg]:shrink-0",
...
function ComboboxChipRemove(props: ComboboxPrimitive.ChipRemove.Props) {
  return (
    <ComboboxPrimitive.ChipRemove
      aria-label="Remove"
      className="h-full shrink-0 cursor-pointer px-1.5 opacity-80 hover:opacity-100 [&_svg:not([class*='size-'])]:size-[calc(--spacing(3.5)+var(--scale-offset)/2)]"

...
function main() {
  ...
  const names = [];
  const descs = [];
  for (let m = nameRegex.exec(content); m !== null; m = nameRegex.exec(content)) {
    names.push(m[1]);
  }
  for (let m = descRegex.exec(content); m !== null; m = descRegex.exec(content)) {
    descs.push(m[1]);
  }
  ...
  for (const type of sortedTypes) {
    ...
    for (const p of particles) {
      lines.push(
        `- **${p.name}** - ${p.desc} | [JSON](${COSS_JSON_BASE_URL}/${p.name}.json)`
      );
    }
    lines.push(``);
  }

---
name: coss-autocomplete
description: Use when implementing searchable suggestion inputs with coss Autocomplete (sync, grouped, limited, async, and form-integrated flows).
...
- **Input affordances**: use `showClear`, `showTrigger`, and `startAddon` on `AutocompleteInput` for discoverability without custom wrappers.
- **Matching behavior**: use `mode="both"` for inline completion and `autoHighlight` when first-result focus should be automatic.
- **Grouped lists**: use `AutocompleteGroup` + `AutocompleteGroupLabel` + `AutocompleteCollection` + `AutocompleteSeparator` for large taxonomies.
- **Large result sets**: use `limit` with `AutocompleteStatus` to expose "+N more" guidance while keeping popup manageable.
- **Async search**: for server-side filtering, use `filter={null}`, control `value`/`onValueChange`, and provide `itemToStringValue` for object results.
- **Form integration**: place `Autocomplete` inside `Field name="..."`; keep `FieldLabel`/`FieldError` semantics and submit via `Form`.

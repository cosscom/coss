...
export function ParticleCardContainer({
  ...
  return (
    <div className="relative flex min-w-0 flex-col before:pointer-events-none before:absolute before:inset-px before:rounded-[calc(var(--radius-xl)-1px)] before:bg-background">
      <div
        className={cn(
          "after:-inset-[5px] after:-z-1 relative flex min-w-0 flex-1 flex-col rounded-xl border bg-muted/72 not-dark:bg-clip-padding shadow-black/5 shadow-sm before:pointer-events-none before:absolute before:inset-0 before:rounded-xl before:shadow-[0_1px_--theme(--color-black/6%)] after:pointer-events-none after:absolute after:rounded-[calc(var(--radius-xl)+4px)] after:border after:border-border/64 dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",

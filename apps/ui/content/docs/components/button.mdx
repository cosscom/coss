...
<Alert>
  <AlertTitle>Use ButtonLink for Links</AlertTitle>
  <AlertDescription>
    For links that look like buttons, use the <a href="/docs/components/button-link" className="underline">ButtonLink</a> component instead of using `Button` with the `render` prop. `ButtonLink` is specifically designed for this use case and renders an `<a>` tag by default.
  </AlertDescription>
</Alert>
...
- For links styled as buttons, use [`ButtonLink`](/docs/components/button-link) instead of `Button` with `render`
...
// For Next.js Link:
import { ButtonLink } from "@/components/ui/button-link"
<ButtonLink render={<Link href="/login" />}>Login</ButtonLink>
```
</span>

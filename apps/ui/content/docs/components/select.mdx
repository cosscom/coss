...
```bash
npx shadcn@latest add https://coss.com/ui/r/select.json
```
...
<span data-lib="radix-ui">
  ```tsx title="shadcn/ui" // [!code word:placeholder="Select a framework"]
  <Select>
    <SelectTrigger>
...
<span data-lib="base-ui">
  ```tsx title="coss.com ui" // [!code word:alignItemWithTrigger={false}] //
  [!code word:items:2]
  <Select
    items={[

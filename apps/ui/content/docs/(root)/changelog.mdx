...
### OTP Field (Base UI 1.6) {#otp-field-base-ui-16}

coss now depends on **`@base-ui/react` 1.6.0**. Base UI graduated OTP Field from preview: the namespace export is **`OTPField`** (not `OTPFieldPreview`). Re-add or update **`@coss/otp-field`** to pick up the registry change.

**Migration** (only if you import Base UI directly or copied an older `otp-field.tsx`):
...
The coss public API is unchanged: **`OTPField`**, **`OTPFieldInput`**, **`OTPFieldSeparator`**, and **`OTPFieldPrimitive`**.

**Agent migration prompt:**

```text
Base UI 1.6 renamed OTPFieldPreview to OTPField. In components/ui/otp-field.tsx (or @coss/otp-field), import { OTPField as OTPFieldPrimitive } from "@base-ui/react/otp-field". Bump @base-ui/react to ^1.6.0. Do not rename coss exports—OTPField, OTPFieldInput, and OTPFieldSeparator stay the same.
```

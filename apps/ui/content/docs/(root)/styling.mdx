...
<TabsList>
  <TabsTab value="init">Init (New Project)</TabsTab>
  <TabsTab value="add">Add (Existing Project)</TabsTab>
  <TabsTab value="manual">Manual</TabsTab>
</TabsList>

<TabsPanel value="init">

**Recommended for new projects.** Initialize with the full coss theme (colors, fonts, base styles):

```bash
npx shadcn@latest init coss/style
```

This gives you everything: semantic colors, font variables (`--font-sans`, `--font-heading`, `--font-mono`), chart and code tokens, plus base layer styles. Components like Dialog and AlertDialog use `font-heading`—the preset provides system font fallbacks, and you can override with `next/font` in your layout.

</TabsPanel>

<TabsPanel value="add">

For existing projects, add the full theme or just colors:

```bash
# Full theme (colors, fonts)
...
These commands will override your existing color variables. We're currently providing a default configuration based on neutral colors, but we plan to add more color variants in the future.

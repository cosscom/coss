...
coss does **not** use the community [`input-otp`](https://github.com/guilhermerodz/input-otp) package. OTP entry is implemented with [Base UI OTP Field](https://base-ui.com/react/components/otp-field) (`OTPFieldPreview`) via the `@coss/otp-field` registry item (`components/ui/otp-field.tsx`).
...
- Add an `aria-label` on each `OTPFieldInput` when there is no visible label associated with the control
